	...
	do {
		for (i32 i = 0; i < countof(gCNR_work); i++) {
			...
			for (i32 j = 0; j < w->cysts_count; j++) {
				...
				/* TODO(rnp): move frame loop out here; append frame number to output */
				Stream buf = arena_stream(a);
				stream_append_s8(&buf, w->method);
				stream_append_byte(&buf, OS_PATH_SEPERATOR);
...
main(i32 argc, char *argv[])
{
	...
	stream_ensure_termination(&wc.path, OS_PATH_SEPERATOR);
